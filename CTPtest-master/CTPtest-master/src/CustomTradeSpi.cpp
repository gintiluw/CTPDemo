	...
	bool bIsLast)
{
	...
	{
		std::cout << "认证失败" << "ErrorID=" << pRspInfo->ErrorID << ", ErrMsg=" << pRspInfo->ErrorID << std::endl;
	}
	...
	bool bIsLast)
{
	if (!isErrorRspInfo(pRspInfo))
	{
		...
		std::cout << "时间： " << pInputOrder->GTDDate << std::endl;
		std::cout << "价格： " << pInputOrder->LimitPrice << std::endl;
		std::cout << "数量： " << pInputOrder->VolumeTotalOriginal << std::endl;  // 现手=后一笔数量-前一笔数量
...
void CustomTradeSpi::reqAuthenticate() {
	...
	std::cerr << "--->>> 发送认证请求：" << ((iResult == 0) ? "成功" : "失败") << std::endl;
}
